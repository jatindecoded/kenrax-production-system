...
function BatchesPageContent() {
  ...
  const filteredBatches = allBatches.filter((batch) => {
    ...
    return (
      batch.batch_code.replace(/\s/g, '').toLowerCase().includes(normalizedSearch) ||
      (batch.part_number && batch.part_number.replace(/\s/g, '').toLowerCase().includes(normalizedSearch))
    );
  });
  ...
  const highlightMatch = (text: string | null | undefined, searchTerm: string) => {
    ...
    if (!normalizedText.toLowerCase().includes(normalizedSearch.toLowerCase())) {
      return text;
    }

    // Find the matching portion and highlight it in the original text
    const regex = new RegExp(`(${normalizedSearch.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})`, 'gi');
    const matchIndex = normalizedText.toLowerCase().indexOf(normalizedSearch.toLowerCase());
      ...
      <div className="border-b border-black">
        <div className="max-w-4xl mx-auto px-4 md:px-6 py-6">
          <div className="flex justify-between items-start gap-4">
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-black tracking-tight mb-1">Production Batches</h1>
            </div>
            <a href="/batches/add" className="px-3 md:px-4 py-2 bg-black text-white font-semibold rounded hover:bg-slate-900 transition-colors text-xs md:text-sm tracking-wide uppercase whitespace-nowrap">
              + New
            </a>
          </div>
        </div>
        ...
        {!loading && filteredBatches.length === 0 && (
          <div className="text-center py-12">
            <p className="text-sm text-slate-600 mb-4">
              {searchTerm ? 'No production batches found.' : 'No production batches yet.'}
            </p>
            <a href="/batches/add" className="inline-block px-4 py-2 bg-black text-white font-semibold rounded hover:bg-slate-900 transition-colors text-sm tracking-wide uppercase">
              Log Batch
            </a>
          </div>
        )}
          ...
          <div className="hidden md:block overflow-x-auto border border-slate-200 rounded">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  <th className="px-4 py-3 text-left font-semibold text-black">Batch Code</th>
                  ...
                  <th className="px-4 py-3 text-right font-semibold text-black">Qty</th>
                  <th className="px-4 py-3 text-left font-semibold text-black">By</th>
                  <th className="px-4 py-3 text-left font-semibold text-black">Line</th>
                  <th className="px-4 py-3 text-left font-semibold text-black">Date</th>
                </tr>
              </thead>
              <tbody>
                {filteredBatches.map((batch, idx) => (
                  <tr key={batch.id} className={idx !== filteredBatches.length - 1 ? 'border-b border-slate-100' : ''}>
                    <td className="px-4 py-3 font-mono font-bold text-black text-base">{highlightMatch(batch.batch_code, searchTerm)}</td>
                    <td className="px-4 py-3 font-bold text-black text-base">{highlightMatch(batch.part_number, searchTerm)}</td>
                    <td className="px-4 py-3 text-black text-xs font-medium">{batch.product_type || '-'}</td>
                    <td className="px-4 py-3 text-right text-black">{batch.quantity}</td>
                    <td className="px-4 py-3 text-black text-xs">{batch.produced_by || '-'}</td>
                    <td className="px-4 py-3 text-black text-xs">{batch.production_line || '-'}</td>
                    <td className="px-4 py-3 text-slate-600 text-xs">{formatDate(batch.created_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <div key={batch.id} className="border border-slate-200 rounded p-4 bg-white">
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-black">{batch.product_type || '-'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600">Qty:</span>
                    <span className="font-medium text-black">{batch.quantity}</span>
                  </div>
                  {batch.produced_by && (
                    <div className="flex justify-between">
                      <span className="text-slate-600">By:</span>
                      <span className="font-medium text-black">{batch.produced_by}</span>
                    </div>
                  )}
